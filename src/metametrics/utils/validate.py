...
def validate_real(arg: float, valid_min: float = None, valid_max: float = None) -> float:
    ...
    """
    if arg is None:
        raise ValueError(f"Invalid argument, argument should not be None!")
    if arg is not None and valid_min is not None and arg < valid_min:
        raise ValueError(f"Invalid argument '{arg}'. Must be greater than or equal to {valid_min}.")
    if arg is not None and valid_max is not None and arg > valid_max:
