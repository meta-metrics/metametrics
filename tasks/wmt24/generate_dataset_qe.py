...
file_dir_path = str(Path(__file__).absolute()).replace("generate_dataset_qe.py","") 
...
REFB_PATH = file_dir_path + "wmt24-metrics-inputs/metrics_inputs/txt/{}/references/{}.{}.ref.refB.{}"
CS_REF_PATH = file_dir_path + "wmt24-metrics-inputs/metrics_inputs/txt/{}/references/{}.{}.ref.ref1.{}"
SYSTEM_FOLDER = file_dir_path + "wmt24-metrics-inputs/metrics_inputs/txt/{}/system_outputs/"
METADATA_PATH = file_dir_path + "wmt24-metrics-inputs/metrics_inputs/txt/{}/metadata/{}.tsv"
METADATA_PATH_challenge_bioMQM = file_dir_path + "wmt24-metrics-inputs/metrics_inputs/txt/{}/metadata/{}.{}.docID.csv" 
...
def segment_level_scoring(samples: Dict[str, List[str]], metric: str):
    ...
    if metric == "COMET-QE":
        ...
        scores = np.random.random(size = len(samples["src"])) * 0
    else:
...
if __name__ == "__main__":
    ...
    segment_data.to_csv("scores_custom/qe-as-a-metric/{}.seg.score".format(args.baseline), index=False, header=True, sep="\t")
    
    system_data = pd.concat(system_data, ignore_index=True)
    system_data.to_csv("scores_custom/qe-as-a-metric/{}.sys.score".format(args.baseline), index=False, header=True, sep="\t")
