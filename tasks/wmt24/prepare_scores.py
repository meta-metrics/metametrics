...
def system_level_scoring(samples: Dict[str, List[str]], metric: str, scores=List[float]):
    ...
    if metric == "chrF":
        ...
        system_score = corpus_meta_metrics(samples["mt"], [samples["ref"],])
...
def corpus_meta_metrics(candidates: list, references: list) -> list:
    # print(">corpus>", len(candidates), len(references), len(references[0]))
    assert len(references) == 1 and len(references[0]) == len(candidates)
    scores = run_sentence_meta_metrics(candidates, references[0])
