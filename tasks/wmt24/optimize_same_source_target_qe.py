...
df = pd.read_csv('all/wmt-mqm-human-evaluation_score_final_scaled.csv')

for langs in [["en","de"],["en","es"],["ja","zh"]]:
    src_lang, tgt_lang = langs
    df = df[(df['lp'].str.contains(src_lang)) | (df['lp'].str.contains(tgt_lang))]
    res = {"mqm": run(df)}
    
    with open(f"same_source_target_{src_lang}-{tgt_lang}_results_with_references_qe.json", "w+") as f:
