...
if __name__ == "__main__":
    ...
    for dataset_name in dataset_names:
        ...
        for metric_id in range(len(metrics_configs)):
            metric_name = metrics_configs[metric_id]
            metric = MetaMetrics([metric_name], weights=[1])
            df[metric_name] = np.array(metric.score(predictions, references, sources))

        df.to_csv(os.path.join(cur_dir, f"output/{dataset_name}_with_{all_metric_names}.csv"), index=False)
